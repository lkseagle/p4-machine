...
def handle_pkt(pkt):
    if ProbeData in pkt:
        ...
        for sw in data_layers:
            print "Switch {} - Port {}: cur_time {} in-packets {} out-packets {} q-Length: {} ".format(sw.swid, sw.port,sw.cur_time, sw.pckcont, sw.enpckcont, sw.qdepth)
        print "\n"
        bedelay=None      
        j=0
        for i in range(len(data_layers)): ####collect node by reversely/
            ...
            if(j>0):
                ...
                droppkt=data_layers[i].enpckcont-data_layers[i-1].pckcont ####befor.ingress---last.egress=link+node drop
